...
module.exports = (function(){
  ...
  Radar.prototype.wrap = function (object, options) {
    ...
    options.methods.forEach(function (v) {
      if (!object[v]) return;
      wrapInterface.apply(self, [object, v, options]);
    });
  ...
  function wrapInterface(object, methodName, options) {
    ...
    (function isolate(object, methodName, method, options, events){
  ...
  function asyncWrap(object, method, args, events) {
    var self = this
    ...
    async.waterfall([
      ...
      function emitAfterEvent() {
        ...
        done.apply(null, _args.slice(0, args.length - 1));
        _args[_args.length - 1].apply(null, [null].concat(_args.slice(0, args.length - 1)));
      }
    ], function (err, result) {
